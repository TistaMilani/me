...
export default function Navbar({ theme = 'dark', onToggleTheme }: NavbarProps) {
    ...
    return (
        <nav className="sticky top-0 z-50 w-full">
            <div className="mx-auto w-full max-w-6xl px-4 py-2 flex flex-col sm:flex-row sm:h-16 items-start sm:items-center justify-between gap-2">
                <div className="py-2">
                ...
                <div className="flex items-center gap-1">
                    {socialLinks.map((link) => (
                        <a
                            ...
                            className={`inline-flex items-center rounded-md p-2 transition focus:outline-none focus:ring-2 ${hoverRing}`}
                            aria-label={link.label}
                        >
                            <img src={link.path} alt={link.label} className={`h-6 w-6 ${iconFilter}`} />
                        </a>
                    ))}
                    ...
                    <button
                        ...
                        className={`ml-2 inline-flex items-center justify-center rounded-full p-1 transition focus:outline-none focus:ring-2 ${hoverRing}`}
                        aria-label="Toggle theme"
                        aria-pressed={theme === 'dark'}
                        title="Toggle theme"
                    >
                        <div className={`relative h-6 w-12 rounded-full ${theme === 'dark' ? 'bg-white/20' : 'bg-black/20'}`}>
                            <svg
                                ...
                                stroke="currentColor"
                                strokeWidth="1.5"
                                className={`absolute left-1 top-1 h-4 w-4 ${theme === 'dark' ? 'text-slate-400' : 'text-black'}`}
                                aria-hidden
                            >
                            ...
                            <svg
                                ...
                                stroke="currentColor"
                                strokeWidth="1.5"
                                className={`absolute right-1 top-1 h-4 w-4 ${theme === 'dark' ? 'text-slate-300' : 'text-slate-500'}`}
                                aria-hidden
                            >
                            ...
                            <div
                                className={`absolute top-0.5 h-5 w-5 rounded-full shadow transition-transform ${theme === 'dark' ? 'translate-x-6 bg-white' : 'translate-x-0 bg-black'}`}
                                aria-hidden
                            />
                        </div>
                    </button>
